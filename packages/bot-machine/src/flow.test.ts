...
import type { AppContext, FlowConfig, FlowState, BotCommand, BotQuery, Component, ActionHandler, MessagePayload } from "./types";
...
const createMockCommand = (executeFn: any = mock(async (input: any) => ({})), inputSchema: any = { parse: mock((x: any) => x) }, outputSchema: any = { parse: mock((x: any) => x) }): BotCommand<any, any> => {
  return {
    _id: "BotCommand",
    input: inputSchema,
    output: outputSchema,
...
const createMockQuery = (executeFn: any = mock(async (input: any) => ({})), inputSchema: any = { parse: mock((x: any) => x) }, outputSchema: any = { parse: mock((x: any) => x) }): BotQuery<any, any> => {
  return {
    _id: "BotQuery",
    input: inputSchema,
    output: outputSchema,
...
describe("FlowController", () => {
  ...
  describe("handle", () => {
    ...
    test("should render the current state if no actions are triggered", async () => {
      ...
      expect(ctx.reply).toHaveBeenCalledWith("Test state", { reply_markup: undefined });
    });
    ...
    test("should update params when processing patterns", async () => {
      const mockCommand = createMockCommand(async (input: any) => ({ result: "success" }));
      const mockComponent = createMockComponent({ text: "Test state" });
    ...
    test("should use function to determine next state based on result", async () => {
      const mockCommand = createMockCommand(async (input: any) => ({ goToNext: true }));
      const mockComponent = createMockComponent({ text: "Next state" });
      ...
      const config: FlowConfig = {
        "initial": {
          ...
          onAction: {
            "conditional": {
              command: mockCommand,
              nextState: (result: any) => result.goToNext ? "nextState" : "stayHere"
            } as ActionHandler
          }
        } as FlowState,
        "nextState": {
  ...
  describe("executeAction", () => {
    ...
    test("should display error message when command execution fails", async () => {
      ...
      expect(ctx.reply).toHaveBeenCalledWith("Произошла ошибка. Попробуйте еще раз.");
    });
  });
  ...
  describe("renderState", () => {
    test("should render a state with onEnter query", async () => {
      ...
      const flowController = new FlowController(config, "testFlow");
      const ctx = createMockContext({ message: { text: "render" } as any });
      ctx.session.flowName = "testFlow";
      ctx.session.flowState = "dataState";

      await flowController.renderState("dataState", ctx);

      expect(executeMock).toHaveBeenCalledWith(undefined, ctx);
      expect(mockComponent).toHaveBeenCalledWith({ count: 42 });
      expect(ctx.reply).toHaveBeenCalledWith("State with data", { reply_markup: undefined });
    });
    ...
    test("should render a state without onEnter query", async () => {
      ...
      await flowController.renderState("simpleState", ctx);

      expect(mockComponent).toHaveBeenCalledWith({});
      expect(ctx.reply).toHaveBeenCalledWith("Simple state", { reply_markup: undefined });
    });

    test("should edit message when update comes from callback query", async () => {
      ...
      const flowController = new FlowController(config, "testFlow");
      const ctx = createMockContext({ 
        callback_query: { data: "click" } as any,
        message: { text: "old" } as any
      });
      ctx.session.flowName = "testFlow";
      ctx.session.flowState = "callbackState";

      await flowController.renderState("callbackState", ctx);

      expect(ctx.answerCallbackQuery).toHaveBeenCalled();
      expect(ctx.editMessageText).toHaveBeenCalledWith("Updated message", { reply_markup: undefined });
    });

    test("should display error message when query execution fails", async () => {
      ...
      await flowController.renderState("failingState", ctx);

      expect(ctx.reply).toHaveBeenCalledWith("Произошла ошибка при отображении экрана.");
    });
  });
  ...
  describe("complex scenarios", () => {
    test("should handle a multi-step flow with data persistence", async () => {
      ...
      const nameQueryExecute = mock(async (input: any) => ({ name: "John", count: 0 }));
      const nameQuery = createMockQuery(nameQueryExecute);
      const setNameCommandExecute = mock(async (input: any) => ({ name: input.name, count: 0 }));
      const setNameCommand = createMockCommand(setNameCommandExecute);
      const incrementCommandExecute = mock(async (input: any) => ({ name: input.text, count: 1 }));
      const incrementCommand = createMockCommand(incrementCommandExecute);
      const finishCommandExecute = mock(async (input: any) => ({ name: "John", count: 1 }));
      ...
      expect(setNameCommandExecute).toHaveBeenCalledWith({ name: "John", text: "John" }, ctx1);
      
      // Step 2: Increment counter via callback
      const ctx2 = createMockContext({ callback_query: { data: "increment" } as any });
      ...
      expect(incrementCommandExecute).toHaveBeenCalledWith({ text: undefined }, ctx2);
      
      // Step 3: Finish flow
      const ctx3 = createMockContext({ callback_query: { data: "done" } as any });
    ...
    test("should handle errors gracefully in multi-step flow without losing progress", async () => {
      const failingCommandExecute = mock(async (input: any) => { throw new Error("Command failed"); });
      const failingCommand = createMockCommand(failingCommandExecute);
      const workingCommandExecute = mock(async (input: any) => ({ result: "success" }));
    ...
    test("should handle session data persistence between states", async () => {
      ...
      const mockCommandExecute = mock(async (input: any) => ({ name: input.text }));
      ...
      const config: FlowConfig = {
        ...
        "step2": {
          component: mockComponent,
          onEnter: createMockQuery(async (input: any) => ({ fromStep1: "value" }))
        } as FlowState
      };
