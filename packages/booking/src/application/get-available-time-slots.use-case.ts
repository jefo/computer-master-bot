...
type GetAvailableTimeSlotsInput = z.infer<
	typeof GetAvailableTimeSlotsInputSchema
>;
...
export const getAvailableTimeSlotsUseCase = async (
	input: unknown,
	...
	return timeSlots
		...
		.map((slot) => {
			const duration =
				(slot.state.end.getTime() - slot.state.start.getTime()) / (1000 * 60); // Convert to minutes
			return {
				id: slot.state.id,
